...
from .compat import is_py3, cmp_to_key
...
class Doudizhu(object):
    ...
    @staticmethod
    def list_playable_cards(cards_candidate):
        playable = []
        candidate_cardmap = str2cardmap(cards_candidate)

        for _, type_data in iter(Doudizhu.TYPE_CARDS.items()):
            for _, card_lists in iter(type_data.items()):
                for w_cards in card_lists:
                    w_cardmap = str2cardmap(w_cards)

                    if Doudizhu.cards_contain(candidate_cardmap, w_cardmap):
                        playable.append(w_cards)
